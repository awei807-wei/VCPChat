...
try {
  console.log(`[SelectionHook] Platform: ${process.platform}, ABI: ${process.versions.modules}, Arch: ${process.arch}`);
  
  if (process.platform !== "win32" && process.platform !== "linux") {
    throw new Error("This module only supports Windows and Linux platforms");
  }
  
  // Try standard gypBuild first
  try {
    nativeModule = gypBuild(path.resolve(__dirname));
    if (nativeModule) console.log("[SelectionHook] Loaded via gypBuild");
  } catch (e) {
    console.log(`[SelectionHook] Local gypBuild failed: ${e.message}, attempting fallback...`);
    
    // Fallback for Windows: Try to load from node_modules if local vendor binary fails
    if (process.platform === "win32") {
      try {
        // Use the original module from node_modules as a safe fallback for Windows
        nativeModule = require("node-gyp-build")(path.join(__dirname, "../../node_modules/selection-hook"));
        if (nativeModule) {
          console.log("[SelectionHook] Windows fallback: Loaded from node_modules successfully");
        }
      } catch (winErr) {
        console.error("[SelectionHook] Windows fallback failed:", winErr.message);
        throw e;
      }
      ...
      // Try current ABI first
      let binPath = path.join(__dirname, 'bin', `linux-${arch}-${abi}`, 'selection-hook.node');
      console.log(`[SelectionHook] Checking path: ${binPath}`);
      
      if (!fs.existsSync(binPath)) {
        console.log(`[SelectionHook] Path not found for ABI ${abi}, falling back to ABI 136...`);
        // Fallback to known available ABI 136 if current ABI is missing
        binPath = path.join(__dirname, 'bin', `linux-x64-136`, 'selection-hook.node');
      }

      if (fs.existsSync(binPath)) {
        try {
          nativeModule = require(binPath);
          console.log(`[SelectionHook] Successfully loaded native module from: ${binPath}`);
        } catch (requireErr) {
          console.error(`[SelectionHook] Failed to require native module at ${binPath}:`, requireErr.message);
          throw e;
        }
      } else {
        console.error(`[SelectionHook] No native module found at ${binPath}`);
        throw e;
      }
    } else {
      throw e;
  ...
  console.error("[SelectionHook] Final load error:", err.message);
}
...
class SelectionHook extends EventEmitter {
  ...
  constructor() {
    if (!nativeModule) {
      throw new Error("Native module failed to load - only works on Windows and Linux");
    }
    super();
  }
