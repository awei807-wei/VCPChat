    ...
    try {
        ...
        if (widgets.length > 0) {
            ...
            widgets.forEach((widget, index) => {
                const created = widget.createdAt
                    ? new Date(widget.createdAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })
                    : '未知';
                const updated = widget.updatedAt
                    ? new Date(widget.updatedAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })
                    : '未知';
                lines.push(`| ${index + 1} | **${widget.name}** | \`${widget.dirName}\` | ${created} | ${updated} |`);
            });
            lines.push('');
            ...
            for (const widget of widgets) {
                ...
                lines.push('');
                lines.push(`- **文件夹**: \`${widget.dirName}/\``);
                lines.push(`- **收藏 ID**: \`${widget.id}\``);
                if (widget.createdAt) {
                    lines.push(`- **创建时间**: ${new Date(widget.createdAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}`);
                }
                if (widget.updatedAt) {
                    lines.push(`- **更新时间**: ${new Date(widget.updatedAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}`);
                }
                lines.push('');
                lines.push('```');
                lines.push(`${widget.dirName}/`);
                for (const file of widget.fileTree) {
                    lines.push(`  ${file}`);
                }
                lines.push('```');
                lines.push('');
...
async function collectFileTree(dirPath, prefix) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.name === 'CATALOG.md') continue;
            if (entry.isDirectory()) {
                ...
                try {
                    ...
                    result.push(`${prefix}${entry.name}  (${formatFileSize(stat.size)})`);
                } catch (e) {
                    result.push(`${prefix}${entry.name}`);
                }
            }
